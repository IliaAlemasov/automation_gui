...
def open_nik(): #запуск плагинов никон
    ...
    bt = check_button_on_screen_on_for('buttons\\update.png', for_confidence=.9) # проверка на окно обновлений и его закрытие
    if bt is not None:
        pyautogui.press('Esc')
    else:
        pass
...
def custom_but_nik(): #кнопка custon nik
    bt = check_button_on_screen('buttons\\custom.png', for_confidence=1)
